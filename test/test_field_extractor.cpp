...
static void CheckParseDataType(FieldType ft, Value& v, const std::string& str_ok,
    ...
    } else if (ft == DOUBLE_VECTOR){
        UT_EXPECT_TRUE(FieldData(fe.GetConstRef(v).AsType<T>()).AsDoubleVector() == fd_ok.AsDoubleVector());
    } else if (ft == BINARY_VECTOR){
        UT_EXPECT_TRUE(FieldData(fe.GetConstRef(v).AsType<T>()).AsBinaryVector() == fd_ok.AsBinaryVector());
    } else {
